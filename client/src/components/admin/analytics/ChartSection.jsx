    ...
    <div className="bg-white dark:bg-gray-800 rounded-2xl p-4 sm:p-6 shadow-sm border border-gray-100 dark:border-gray-700">
      ...
      {/*
        ...
        parents like motion.div). Fix:
         - Give the wrapper an explicit minHeight so it always has a non-zero
           size even before the ResizeObserver fires.
         - Pass `aspect={2}` so the container derives its height from the
           measured width via the ratio — this gives Recharts a definite
           dimension even when `height="100%"` would otherwise resolve to 0.

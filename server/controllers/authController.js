...
exports.sendRegistrationOtp = async (req, res, next) => {
  try {
    ...
    const { subject: emailSubject, html: emailHtml } = getAuthEmail('email-verification', name.trim() || 'there', otp);
...
exports.sendEmailUpdateOtp = async (req, res, next) => {
  try {
    ...
    const { subject: updateSubject, html: updateHtml } = getAuthEmail('email-update', user.name || 'there', otp);
    await sendEmail({
...
exports.forgotPassword = async (req, res, next) => {
  try {
    ...
    // Send email
    const { subject: resetSubject, html: resetHtml } = getAuthEmail('reset-password', user.name, otp);
    await sendEmail({
      email: user.email,
